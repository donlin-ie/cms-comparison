...
const CompareOneField = ({ field, allCms, svgFileChecked, svgFileArrowLeft, svgFileQuestion }) => {
  return (
    <tr className={field.rowClass}>
      <th scope="row">
        {field.description}
  { (field.slideIndex && field.slideIndex !== '') ? (<a href={ `/slide/${field.slideIndex}/` } >#{field.slideIndex}</a>) : null }
      </th>
      {allCms.map(({ node }) => {
        const cmsFieldsRaw = (node.comparisonFieldValues && node.comparisonFieldValues.internal.content) ? node.comparisonFieldValues.internal.content : null;
        const cmsFields = (cmsFieldsRaw) ? JSON.parse(cmsFieldsRaw) : {};
        const cmsValue = (cmsFields && cmsFields[field.name]) ? cmsFields[field.name] : "";
        if (Array.isArray(cmsValue)) {
          //array 
          return (
            <td key={field.id + node.name } >{ ReactHtmlParser(cmsValue.join('<br/>'))}</td>
          )
        }
        else if (typeof cmsValue === "boolean") {
          //boolean
          return (
            <td key={field.id + node.name } >{ (cmsValue) ? (<img src={svgFileChecked.publicURL} alt="" width="20" />) : "No"}</td>
          )
        }
        else if (cmsValue === "<-") {
          //boolean
          return (
            <td key={field.id + node.name } >{ (cmsValue) ? (<img src={svgFileArrowLeft.publicURL} alt="" width="20" />) : "No"}</td>
          )
        }
        else if (cmsValue === "?") {
          //boolean
          return (
            <td key={field.id + node.name } >{ (cmsValue) ? (<img src={svgFileQuestion.publicURL} alt="" width="20" />) : "No"}</td>
          )
        }
        else {
          ...
          //other
          return (
            <td key={field.id + node.name } >{ ReactHtmlParser(cmsValue) }</td>
          )
        }
